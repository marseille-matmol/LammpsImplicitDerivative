...
def run_minimization(method,
    ...
    datafile_path_easy = os.path.join(data_path, 'dislo_sub_easy_core.lammps-data')
    datafile_path_hard = os.path.join(data_path, 'dislo_sub_hard_core.lammps-data')

    mpi_print('Dislo easy core initial relaxation...', comm=comm)
    with trun.add('easy core init'):
        dislo_easy = DisloSub(snapcoeff_filename='WX.snapcoeff',
                              snapparam_filename='WX.snapparam',
                              datafile=datafile_path_easy,
                              logname='dislo_sub.log',
                              comm=comm,
                              verbose=True)

    mpi_print('Dislo hard core initialization (no relaxation)...', comm=comm)
    with trun.add('hard core init'):
        dislo_hard = DisloSub(snapcoeff_filename='WX.snapcoeff',
                              snapparam_filename='WX.snapparam',
                              datafile=datafile_path_hard,
                              minimize=False,
                              logname='dislo_sub_hard_init.log',
                              comm=comm)
    ...
    #X_hard = dislo_easy.minimum_image(X_hard)
